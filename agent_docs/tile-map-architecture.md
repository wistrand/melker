...
2. svgOverlay <path> elements — geo-anchored vector paths drawn to pixel buffer
3. onOverlay(canvas, bounds, geo) — app draws markers, routes, etc. to pixel buffer
4. onShader runs per-pixel over combined buffer — post-processing
5. Buffer rendered to terminal (sextant/sixel/kitty/halfblock/etc.)
6. svgOverlay <text> elements — rendered as terminal characters on top of canvas
7. Text labels from drawText/drawTextColor calls — rendered on top of canvas
```
