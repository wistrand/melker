...
| `earthquake-dashboard.melker` (showcase/) | `<tile-map>` with `onOverlay` for geo markers, `setSvgOverlay()` for tectonic plates, fetch API, `onTooltip`, zoom-dependent thresholds, `$melker.cache` for GeoJSON caching with versioned keys |
...
| `tile-map-svg-paths.melker` (components/) | `<tile-map>` `setSvgOverlay()` layers, declarative `<path>` and `<text>` elements with lat/lon |
...
| `data-heatmap-metaballs.melker` (components/) | Metaball field visualization, auto-isolines (isolineCount/isolineMode), animated blobs |
| `data-tree.melker` (components/) | Hierarchical tree, single-column, multi-column, selection, expand/collapse, tooltip |
